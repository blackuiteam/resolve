...
function MediaPool() {
	...
	}, [toggleRecSettings, toggleCameraSource, toggleAudioSource]);
	...
	const onButtonClick = () => {
		// inputEl.current.paused ? inputEl.current.play() : inputEl.current.pause();
		// if(!inputEl.current.paused) {
		// 	setRunning(!running);
		// }
		// else {
		// 	setRunning(false);
		// 	setTime(0);
		// 	inputEl.current.currentTime = 0;
		// }
		setRec(!toggleRec);
		...
		// if(toggleVideoIcon === false && toggleAudioIcon === false || toggleVideoIcon === true && toggleAudioIcon === false) {
	...
	function handleBackLight() {
		...
		if(!backlight) {
			// console.log("ON..."+backlight+ '...'+toggleFrontLight);
			setFrontLight(false);
			setPicker(0);
		} else {
			// console.log("OFF.."+backlight+'...'+toggleFrontLight);
			setFrontLight(true);
		}
	}
